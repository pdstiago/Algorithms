template <class T> struct SegP{
    int n, cont=0, t=0, neutral=0;
    vector<T> tree, lazy;
    vector<int> esq, dir, ver;
 
    SegP(int n, int q) : n(n), tree(4*(n+1)+18*(q+1)), esq(4*(n+1)+18*(q+1)), dir(4*(n+1)+18*(q+1)), 
    lazy(4*(n+1)+18*(q+1)), ver(q+2) {}
    ...
    int newlazy(int node, int l, int r, T val){
        int no = ++cont;
        esq[no] = esq[node];
        dir[no] = dir[node];
        lazy[no] = lazy[node];
